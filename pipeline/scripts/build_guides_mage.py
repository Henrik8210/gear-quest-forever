"""guides_mage.json from the one Wowhead Classic warlock DPS guide.
...
DEMOTE=re.compile(r"\b(2 set bonus|8/8 t2|offset|sustain|undead|hit|mana sustain)\b")
# A row tagged with a SPEC ("Optional - Frost") is that spec's answer and one tier worse
# for the other two -- the same treatment the rogue guide's "(Swords)" sections get.
SPEC_TAG=re.compile(r"\b(frost|fire|arcane)\b")
NEUTRAL=re.compile(r"\b(pvp|throughput|legendary)\b")
def tier(rank):
    l=re.sub(r"\s+"," ",rank.strip().lower())
    tag=SPEC_TAG.search(l)
    demote=1 if DEMOTE.search(l) else 0
    core=re.sub(r"\s+"," ",NEUTRAL.sub("",SPEC_TAG.sub("",DEMOTE.sub("",l)))).strip(" -")
    if core not in BASE: raise SystemExit("unknown rank: %r (core %r)"%(rank,core))
    return BASE[core]+demote, (tag.group(1) if tag else None)
...
for line in open("mage_raw.txt",encoding="utf-8").read().strip().splitlines():
    slot,rank,name=[x.strip() for x in line.split("|",2)]
    sl=SLOT[slot.lower()]; t,tag=tier(rank)
    # A row can name TWO items -- "Field Marshal's Coronet / Warlord's Silk Cowl" is the
    # Alliance/Horde PvP pair for one slot. 19 rows on this page do. Each half is its own
    # pick; the faction gate in score.py then shows each side only its own.
    for one in [x.strip() for x in name.split(" / ")]:
        nm,roll=resolve(one)
        if nm is None: unmatched.append(one); continue
        if roll=="~": nearmiss.append((one,nm))
        elif roll: rolled.append((nm,roll))
        for spec in SPECS:
            tt = t if (tag is None or tag==spec) else t+1
...
json.dump({s:{sl:[[t,n] for t,n in v] for sl,v in d.items()} for s,d in out.items()},
          open("guides_mage.json","w"),indent=1,ensure_ascii=False)
